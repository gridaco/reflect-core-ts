 ...
 * @param format
 */
export function convertReflectColorToUniversal(
    color?: Color,
    format = ColorFormat.hex,
    inputFormat = ColorFormat.unknown
): string | undefined {
        ...
        if ("a" in (color as any)) {
            switch (format) {
                ...
                case ColorFormat.hex6:
                    console.info(
                        `while converting Color, the givven color ${color} is type of RGBA, but tring to convert it as hex6. dropping the alpha value.`
                    );
...
export function rgbaTo8Hex(
    ...
    try {
        ...
        console.warn(
            "error while converting color to hex string, returning fallback #FFFFFF"
        );
        return "#FFFFFF";
    }
}
...
/**
 * converts (RGBAF) 1,1,1,1 to (RGBA) 255,255,255,255
 * @param rgbaf RGBA color in float format e.g. white is 1,1,1,1
 * @returns
 */
export function rgbafToRgba(rgbaf: RGBAF): RGBA {
    return {
        r: rgbaf.r * 255,
        g: rgbaf.g * 255,
        b: rgbaf.b * 255,
